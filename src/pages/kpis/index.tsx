...
import { getKpis, deleteKpiById } from 'apiSdk/kpis';
import { KpiInterface } from 'interfaces/kpi';

type ColumnType = ColumnDef<KpiInterface, unknown>;
...
export function KpiListPage(props: KpiListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<KpiInterface>>(
    () => `/kpis?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: KpiInterface) => {
    if (hasAccess('kpi', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/kpis/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <>
          {hasAccess('kpi', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/kpis/edit/${record.id}`} passHref legacyBehavior>
              <Button
          ...
          {hasAccess('kpi', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="kpi" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Kpi
        </Text>
        {hasAccess('kpi', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/kpis/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
